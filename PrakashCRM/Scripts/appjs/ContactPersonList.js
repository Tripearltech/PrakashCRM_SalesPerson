...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPContacts/GetContactPersonListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    var rowData = "<tr><td></td><td><a href='/SPContacts/CompanyContactCard?No=" + item.Company_No +"'><i class='bx bxs-edit'></i></a></td><td>" + item.Name + "</td><td>" + item.Company_Name + "</td><td>" + item.E_Mail + "</td><td>" + item.Mobile_Phone_No + "</td></tr>";
...
function pageMe() {
    ...
    var numItems = $('#hdnCPersonCount').val(); //32;// children.length;
...
function exportGridData(skip, top, firsload, orderBy, orderDir, filter) {
    $.ajax(
        {
            url: '/SPContacts/ExportContactListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            contentType: 'application/json',
            success: function (data) {
                //alert(data);
