...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    var apiUrl = $('#getServiceApiUrl').val() + 'SPWarehouseTransfer/';
    $.get(apiUrl + 'GetApiRecordsCount?apiEndPointName=TransferOrderDotNetAPI&filter=' + filter, function (data) {
        $('#hdnWTITaskCount').val(data);
    });

    $.ajax(
        {
            url: '/SPWarehouseTransfer/GetWarehouseIncomingTaskListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
