...
$(document).ready(function () {
    ...
    $('#ddlField').change(function () {
        ...
        if ($('#ddlField').val() == "Expected_Receipt_Date") {
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetApiRecordsCount?apiEndPointName=PurchaseOrderCardDotNetAPI&filter=' + filter, function (data) {
        $('#hdnWPITaskCount').val(data);
    });

    $.ajax(
        {
            url: '/SPWarehousePurchase/GetWarehouseIncomingTaskListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    var rowData = "<tr><td></td><td><input type='checkbox' class='form-check-input' value='" + item.SystemId + "' /></td><td>" + item.No + "</td><td>" + item.Buy_from_Vendor_No + "</td><td>" + item.Buy_From_Vendor_Name + "</td><td>" + item.Location_Code + "</td><td>" + item.Shipment_Date + "</td><td><a class='ViewInqProdCls' onclick='ShowPurchaseLines(\"" + item.No + "\")'><i class='bx bx-show'></i></a></td></tr>";// 
...
function pageMe() {
    ...
    var numItems = $('#hdnWPITaskCount').val(); //32;// children.length;
