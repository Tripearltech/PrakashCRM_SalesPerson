...
$(document).ready(function () {
    ...
    filter += "Closed eq " + $('#ddlStatus').val();
    ...
    $('#btnSearch').click(function () {
        ...
        if ($('#ddlStatus').val() != "-1" && filter != "") {
            filter += " and Closed eq " + $('#ddlStatus').val();
        }
    ...
    $('#btnExport').click(function () {
        ...
        if ($('#ddlField').val() == "Posting_Date" || $('#ddlField').val() == "Due_Date") {
            ...
            if ($('#ddlField').val() != "Closed") {
                ...
                switch ($('#ddlOperator').val()) {
                    case 'Equal':
                        if ($('#txtSearch').val().toLowerCase() == "paid") {
                            filter = $('#ddlField').val() + ' eq true';
                            break;
                        }
                        if ($('#txtSearch').val().toLowerCase() == "unpaid") {
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPPostedSalesInvoice/GetPostedSalesInvoiceListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    ...
                    if (item.Closed == "true") {
                        ...
                        rowData = rowData + "<span class='badge bg-danger'>Unpaid</span>";
                    }
...
function exportGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPPostedSalesInvoice/ExportListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            contentType: 'application/json',
            success: function (data) {
                //alert(data);
                ...
                if (data.fileName != "") {
                    ...
                    window.location.href = "/SPPostedSalesInvoice/Download?file=" + data.fileName;
                }
            },
