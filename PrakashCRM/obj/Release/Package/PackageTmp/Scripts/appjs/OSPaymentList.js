    ...
    $('#btnSearch').click(function () {
        ...
        if ($('#ddlField').val() == "Posting_Date" || $('#ddlField').val() == "Due_Date") {
            ...
            switch ($('#ddlOperator').val()) {
                case 'Equal':
                    if ($('#ddlField').val() != "Amount_LCY" && $('#ddlField').val() != "Remaining_Amt_LCY") {
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPOutstandingPayment/GetOutstandingPaymentListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
...
function pageMe() {
    ...
    var numItems = $('#hdnSPOSPayCount').val(); //32;// children.length;
...
function exportGridData(skip, top, firsload, orderBy, orderDir, filter) {
    $.ajax(
        {
            url: '/SPOutstandingPayment/ExportListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            contentType: 'application/json',
            success: function (data) {
                //alert(data);
                ...
                if (data.fileName != "") {
                    ...
                    window.location.href = "/SPOutstandingPayment/Download?file=" + data.fileName;
                }
            },
