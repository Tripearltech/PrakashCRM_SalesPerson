...
$(document).ready(function () {
    ...
    $('#btnSearch').click(function () {
        ...
        if ($('#txtFromDate').val() == "" || $('#txtToDate').val() == "") {
            ...
            filter = "Visit_Date" + ' ge ' + $('#txtFromDate').val() + ' and ' + "Visit_Date" + ' le ' + $('#txtToDate').val();
        }

        if ($('#ddlSalesPerson').val() != "") {
            filter += " and User_Code eq '" + $('#ddlSalesPerson').val() + "'";
        }
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPVisitEntry/GetDailyVisitExpenseReport?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
...
function pageMe() {
    ...
    var numItems = $('#hfDVExpenseCount').val(); //32;// children.length;
...
//let time24 = "15:20:00";
//console.log(convertToAmPm(time24)); // Output: "3:20 PM"