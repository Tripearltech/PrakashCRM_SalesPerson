...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetAcceptedApiRecordsCount?SPCode=' + $('#hdnLoggedInUserSPCode').val() + '&apiEndPointName=PurchaseOrderCardDotNetAPI&filter=' + filter, function (data) {
        $('#hdnWSATaskCount').val(data);
    });

    $.ajax(
        {
            url: '/SPWarehousePurchase/GetWarehouseAcceptedTaskListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
