...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetAcceptedApiRecordsCount?SPCode=' + $('#hdnLoggedInUserSPCode').val() + '&apiEndPointName=SalesOrderCardDotNetAPI&filter=' + filter, function (data) {
        $('#hdnWSATaskCount').val(data);
    });

    $.ajax(
        {
            url: '/SPWarehouseSales/GetWarehouseAcceptedTaskListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
