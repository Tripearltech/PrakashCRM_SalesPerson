...
$(document).ready(function () {
    ...
    $('#btnSearch').click(function () {
        ...
        //else {
            ...
            //if ($('#ddlYear').val() == "-1" || $('#ddlField').val() == "-1" || $('#ddlOperator').val() == "-1" || $('#txtSearch').val() == "") {
        ...
        if ($('#ddlYear').val() != "-1") {
            ...
            if ($('#ddlField').val() == "-1" && ($('#ddlOperator').val() == "-1" || $('#ddlOperator').val() == "Contains") && $('#txtSearch').val() == "") {
                filter = "Year eq \'" + $('#ddlYear').val() + "\'";
                flag = true;
            }
            else if ($('#ddlField').val() != "-1" && ($('#ddlOperator').val() == "-1" || $('#txtSearch').val() == "")) {
                var msg = "Please select operator and fill search text";
                ShowErrMsg(msg);
            }
            else {
                switch ($('#ddlOperator').val()) {
                    case 'Equal':
                        if ($('#ddlField').val() != "No_Of_Visit") {
                ...
                filter += " and Year eq \'" + $('#ddlYear').val() + "\'";
                flag = true;
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetApiRecordsCount?SPCode=' + $('#hfSPCode').val() + '&Page=YearMonthPlan&apiEndPointName=YearlyMonthVisitDotNetAPI&filter=' + filter, function (data) {
        $('#hdnYMPCount').val(data);
    });

    $.ajax(
        {
            url: '/SPVisitEntry/GetYearMonthPlanListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    var rowData = "<tr><td></td><td><a href='/SPVisitEntry/YearMonthVisitPlan?No=" + item.No + "'><i class='bx bxs-edit'></i></a></td><td>" + item.Year + "</td><td>" + item.Visit_Type_Name + "</td><td>" + item.Visit_Sub_Type_Name + "</td><td>" + item.No_of_Visit + "</td>" +
                        "<td>" + item.No_of_Actual_Visit + "</td></tr>";
...
function pageMe() {
    ...
    var numItems = $('#hdnYMPCount').val(); //32;// children.length;
