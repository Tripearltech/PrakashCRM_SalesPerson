...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.ajax(
        {
            url: '/SPSiteActivity/GetSiteActivityListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
...
function pageMe() {
    ...
    var numItems = $('#hdnSPSACount').val(); //32;// children.length;
...
function exportGridData(skip, top, firsload, orderBy, orderDir, filter) {
    $.ajax(
        {
            url: '/SPSiteActivity/ExportListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            contentType: 'application/json',
            success: function (data) {
                ...
                if (data.fileName != "") {
                    ...
                    window.location.href = "/SPSiteActivity/Download?file=" + data.fileName;
                }
            },
