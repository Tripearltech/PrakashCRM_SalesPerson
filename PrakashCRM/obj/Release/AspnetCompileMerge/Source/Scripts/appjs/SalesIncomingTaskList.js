...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetApiRecordsCount?apiEndPointName=SalesOrderCardDotNetAPI&filter=' + filter, function (data) {
        $('#hdnWSITaskCount').val(data);
    });

    $.ajax(
        {
            url: '/SPWarehouseSales/GetWarehouseIncomingTaskListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    var rowData = "<tr><td></td><td><input type='checkbox' class='form-check-input' value='" + item.SystemId + "' /></td><td>" + item.No + "</td><td>" + item.Sell_to_Customer_No + "</td><td>" + item.Sell_to_Customer_Name + "</td><td>" + item.Location_Code + "</td><td>" + item.Shipment_Date + "</td><td><a class='ViewInqProdCls' onclick='ShowSalesLines(\"" + item.No + "\")'><i class='bx bx-show'></i></a></td></tr>";// 
                ...
                $.post(
                    ...
                    function (data) {
                        ...
                        if (data) {
                            ...
                            var actionMsg = "Task Accepted Successfully";
