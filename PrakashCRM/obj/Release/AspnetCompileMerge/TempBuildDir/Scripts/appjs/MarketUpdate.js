...
var apiUrl = $('#getServiceApiUrl').val() + 'SPMarketUpdate/';
...
$(document).ready(function () {
    ...
    $('#btnSearch').click(function () {
        ...
        if ($('#txtFromDate').val() == "" || $('#txtToDate').val() == "") {
            ...
            filter = "Update_Date" + ' ge ' + $('#txtFromDate').val() + ' and ' + "Update_Date" + ' le ' + $('#txtToDate').val();
        }
...
function bindGridData(skip, top, firsload, orderBy, orderDir, filter) {
    ...
    $.get(apiUrl + 'GetApiRecordsCount?SPNo=' + $('#hfSPNo').val() + '&apiEndPointName=MarketUpdateDotNetAPI&filter=' + filter, function (data) {
        $('#hfMUCount').val(data);
    });

    $.ajax(
        {
            url: '/SPMarketUpdate/GetMarketUpdateListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    var rowData = "<tr><td>" + item.Update_Date + "</td><td>" + item.Update + "</td><td>" + item.Employee_Name + "</td></tr>";
...
function pageMe() {
    ...
    var numItems = $('#hfMUCount').val(); //32;// children.length;
...
function exportGridData(skip, top, firsload, orderBy, orderDir, filter) {
    $.ajax(
        {
            url: '/SPMarketUpdate/ExportListData?orderBy=' + orderBy + '&orderDir=' + orderDir + '&filter=' + filter + '&skip=' + skip + '&top=' + top,
            type: 'GET',
            contentType: 'application/json',
            success: function (data) {
                //alert(data);
                ...
                if (data.fileName != "") {
                    ...
                    window.location.href = "/SPMarketUpdate/Download?file=" + data.fileName;
                }
            },
